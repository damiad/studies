        ...
        if (znak == '-' || znak == '+') {
            ...
            if (wspolczynnik_czy_potega == false) {
                ...
                if (potega == 0) potega = 10;
                   ...
                   'x') { // wtedy zaczynamy zliczać potęgę zamiast współczynnika;
            wspolczynnik_czy_potega = true;
            potega = 1;
            if (wspolczynnik == 0) wspolczynnik = 1;
        }
    }
    Vielomian[potega] = (plus_minus) *
                        wspolczynnik; //teraz wstawiamy  ostatni wspólczynnik w odpowiedniej potędze po wyjściu z while
}
...
void mnozenie(int w[], int v[]) {
    ...
    for (int i = 0; i <= STOPIEN; i++) {
        for (int j = 0; j <= STOPIEN - i;
                j++) //Bo mamy dane że wynikowy wielomian ma stopień co najwyżej 10
            w[i + j] += (copyw[i] * v[j]);
    }
}
...
void wypisz(int w[]) { //pomiń 1 lub -1 w wypisywaniu
    bool czy_pierwszy =
        true; //Pierwszy niezerowy współczynnik wielomianu (true to pierwszy)
...
int main(void) {
    ...
    while ((dzialanie = getchar()) != '.') {
        ...
        if (dzialanie == '*') mnozenie(Wielomian,
                                           Vielomian); // wykonaj dodawanie albo mnozenie
